...
export default function BookingDialog() {
  ...
  const { isOpen, setIsOpen, selectedServiceId } = useBookingDialog();
  ...
  // Effect to auto-select service when dialog opens with a selectedServiceId
  useEffect(() => {
    if (selectedServiceId && services.length > 0) {
        handleAddService(selectedServiceId);
    }
